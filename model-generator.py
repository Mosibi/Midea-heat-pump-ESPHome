    ...
    if 'remove' in overrides:
        for component_type, remove_items in overrides['remove'].items():
            ...
            if not isinstance(remove_identifiers, list):
                print(f"Error: 'remove' section for '{component_type}' must be a list of identifiers.")
                continue
            if component_type in base and isinstance(base[component_type], list):
                base[component_type] = [item for item in base[component_type] if item.get('id') not in remove_identifiers]

    # MODIFY per component type
    if 'modify' in overrides:
        for component_type, modify_identifiers in overrides['modify'].items():
            if not isinstance(modify_identifiers, list):
            ...
            for mod in modify_identifiers:
                ...
                for item in base[component_type]:
                    if item.get('id') == entity_id:
                        ...
                        base[component_type].insert(item_index, mod)
                        modified = True
                        break
                if not modified:
                    print(f"Warning: id {entity_id} not found to modify in '{component_type}'.")
        ...
        for component_type, items in overrides['add'].items():
            ...
            for new_item in items:
                ...
                if addr in existing_addresses:
                    print(f"Warning: id {id} already exists in '{component_type}', skipping.")
                else:
